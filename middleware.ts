...
export function middleware(request: NextRequest) {
    ...
    // paths that don't exist, causing 404s. Same for /ingest (proxy do PostHog via rewrites).
    const isApiRoute = request.nextUrl.pathname.startsWith('/api/')
    const isPostHogProxy = request.nextUrl.pathname.startsWith('/ingest')

    const response = isApiRoute || isPostHogProxy
        ? NextResponse.next()
